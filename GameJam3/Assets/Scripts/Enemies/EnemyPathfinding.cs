...
public class EnemyPathfinding : AIBase
{
    ...
    //mogelijke missie targets
    GameObject player;
    //Afstand tussen player en zombie
    Vector2 direction;
    float distance;
    // Afstand tussen civilian en zombie
    Vector2 civilianDirection;
    float distanceCivilian;
    //Afstand tussen defendpoint en zombie
    Vector2 defendPointDirection;
    float defendPointDistance;
    // afstand waarbij de zombie de player niet meer volgt
    public float chaseRange;
    //Afstand waarbij de idlewaypoint wordt gekozen
    public float range;
    // kijkt of het de target moet veranderen
    bool civilianInRange, playerInRange = true, placeFound = true;
    //tijd om een nieuwe rarandom idlewaypoint te kiezen 
    int waypointTimer;
    // Hoe lang de civilian met iets moet colliden tot dat zij naar een waypoint gaat.
    float time = 3, timeReset = 1, resetTime = 3;
    //afstand tussen civilian en waypoint
    float dToTarget;
    // op welke afstand de civilian de waypoints moet negeren
    ...
    void Init()
    {
        ...
        if (!playerInRange && !civilianInRange)
        {
            MoveToRandomPoint();
        }
    }
    ...
    {
        var randomWaypoint = (int)Random.Range(0, IdleWaypoints.waypoints.Length);
        if (Vector3.Distance(gameObject.transform.position, IdleWaypoints.waypoints[randomWaypoint].transform.position) < range)
        {
            target = IdleWaypoints.waypoints[randomWaypoint];
            placeFound = true;
        }
        else
        {
    ...
    {
        //Als de civilian botst dan voert ze de functie uit na zo veel tijd 
    ...
    // pakt de dichtstbijzijnde waypoint(niet de player) die niet de currentwaypoint is en maakt het de target.
    void ChooseTarget()
    {
        float closestDistance = Mathf.Infinity;
        currentWaypoint = target;
        for (int i = 0; i < PathfindingWaypoints.waypoints; i++)
        {
            if (PathfindingWaypoints.pathfindingWaypoints[i] != currentWaypoint && PathfindingWaypoints.pathfindingWaypoints[i] != null)
            {
                float dToTarget = Vector3.Distance(PathfindingWaypoints.pathfindingWaypoints[i].transform.position, transform.position);
                if (dToTarget < closestDistance && dToTarget >= targetRange)
                {
                    closestDistance = dToTarget;
                    target = PathfindingWaypoints.pathfindingWaypoints[i];
                }
            }
        }
        time = resetTime;//Langere tijd zodat het niet vastloopt in een lange collision
    }
    //Pakt de dichtstbijzijnde waypoint of player
    Transform GetClosestWaypoint(Transform[] waypoints)
    {
        ...
        {
            if (PathfindingWaypoints.pathfindingWaypoints[i] != null && i >= PathfindingWaypoints.waypoints)
            {

                float dToTarget = Vector3.Distance(waypoints[i].transform.position, transform.position);
