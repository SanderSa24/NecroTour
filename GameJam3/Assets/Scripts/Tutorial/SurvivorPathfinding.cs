...
public class SurvivorPathfinding : AIBase
{
    ...
    public static bool endPoint = false;
    public Transform target;
    public Transform currentWaypoint, previousWaypoint;
    GameObject player, safePlace;
    public static int j = 0;
    public int waypointMax;
    ...
    float dToTarget, dToPlayer;
    // op welke afstand de civilian de waypoints moet negeren
    public float targetRange = 0.5f, playerRange = 4;
    float beginPlayerRange;
    // collision check
    ...
    void Init()
    {
        ...
        safePlace = GameObject.FindGameObjectWithTag("Bus");
    }
    ...
    protected override void MovementUpdate(float deltaTime)
    {
        ...
        if (playerRange <= 0) playerRange = beginPlayerRange;
    ...
    void ChooseTarget()
    {
        ...
        {
            if (TutorialWaypoints.pathfindingWaypoints[i] != currentWaypoint)
            {
                float dToTarget = Vector3.Distance(TutorialWaypoints.pathfindingWaypoints[i].transform.position, transform.position);
                if (dToTarget < closestDistance && dToTarget >= targetRange)
                {
                    closestDistance = dToTarget;
                    // j = i;
                    target = TutorialWaypoints.pathfindingWaypoints[i];
                }
            }
        }
    ...
    void CollisionTarget()
    {
        ...
        {
            if (PathfindingWaypoints.pathfindingWaypoints[i] != currentWaypoint)
            {
